...
criterion = "mse"
abu = AbuTree2(criterion = criterion,min_samples_leaf=30, adaptive_complexity=True,alpha=1.1)
np.random.seed(SEED)
n = 100
noise = 2
...
#-----------
f, ((ax1,ax2,ax3,ax4),( ax5, ax6,ax7,ax8), ( ax9, ax10,ax11,ax12)) = plt.subplots(3, 4, figsize = (16, 8),dpi=100)
...
# Plot predictions with uncertainty and color-coded fill
for i in range(len(x_test) - 1):
    ax9.fill_betweenx(y =[-7, np.max(y_hat) + 5], x1=x_test[i], x2=x_test[i + 1], color=cmap_var(norm_var(var_y_hat[i])))

ax9.plot(x_test, y_hat, label='Predicted Y', c = "black")
ax9.scatter(x_test, y_test, c = "red",s=1)
sm = ScalarMappable(cmap=cmap_var, norm=norm_var)
sm.set_array([])  # dummy empty array for the colorbar
cbar = plt.colorbar(sm, label='Prediction variance', ax=ax9)
...
for i in range(len(x_test) - 1):
    ax10.fill_betweenx(y =[-7, np.max(y_hat) + 5], x1=x_test[i], x2=x_test[i + 1], color=cmap_y_var(norm_y_var(var_y[i])))



ax10.plot(x_test, y_hat, label='Predicted Y', c = "black")
ax10.scatter(x_test, y_test, c = "red",s=1)

#plt.fill_between(x.ravel(), lower_bound, upper_bound, alpha=0.3, label='Uncertainty')
# Add colorbar
sm_y_var  = ScalarMappable(cmap=cmap_y_var, norm=norm_y_var)
sm_y_var.set_array([])  # dummy empty array for the colorbar
cbar = plt.colorbar(sm_y_var, label='response variance', ax=ax10)

#-----------


for i in range(len(x_test) - 1):
    ax11.fill_betweenx(y =[-7, np.max(y_hat) + 5], x1=x_test[i], x2=x_test[i + 1], color=cmap_gamma(norm_gamma (gamma[i])))


norm_gamma  = Normalize(vmin=0, vmax=np.max(gamma))

ax11.plot(x_test, y_hat, label='Predicted Y', c = "black")
ax11.scatter(x_test, y_test, c = "red",s=1)

#plt.fill_between(x.ravel(), lower_bound, upper_bound, alpha=0.3, label='Uncertainty')
# Add colorbar
sm_gamma  = ScalarMappable(cmap=cmap_gamma, norm=norm_gamma)
sm_gamma.set_array([])  # dummy empty array for the colorbar
cbar = plt.colorbar(sm_gamma, label=r'$\gamma$', ax=ax11)
...
norm_gamma_tree  = Normalize(0, vmax=np.max(gamma_tree))#vmin=np.min(gamma_tree)
print(np.min(gamma_tree))
for i in range(len(x_test) - 1):
    ax12.fill_betweenx(y =[-7, np.max(y_hat) + 5], x1=x_test[i], x2=x_test[i + 1], color=cmap_gamma(norm_gamma_tree (gamma_tree[i])))




ax12.plot(x_test, y_hat, label='Predicted Y', c = "black")
ax12.scatter(x_test, y_test, c = "red",s=1)

#plt.fill_between(x.ravel(), lower_bound, upper_bound, alpha=0.3, label='Uncertainty')
# Add colorbar
sm_gamma  = ScalarMappable(cmap=cmap_gamma, norm=norm_gamma_tree)
sm_gamma.set_array([])  # dummy empty array for the colorbar
cbar = plt.colorbar(sm_gamma, label=r'$\frac{\gamma}{node\;obs}$', ax=ax12)
