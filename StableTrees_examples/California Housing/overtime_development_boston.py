...
data = datapreprocess.data_preperation("Boston") # load dataset
...
colors = {"baseline":'#1f77b4',"NU":"#D55E00", "SL":"#CC79A7", "SL2":"#a36085","SL3":"#8e5474","SL4":"#8e5474",
            "TR":"#009E73", 
            "ABU":"#F0E442",
            "ABU1":"#F0E442",
            "ABU2":"#F0E442",
...
models = {  
                ...
                "SL4":StabilityRegularization(criterion = criterion,min_samples_leaf=5, adaptive_complexity=True,gamma=0.9),
                "ABU": AbuTree2(criterion = criterion,min_samples_leaf=5,adaptive_complexity=True),
                "ABU2": AbuTree2(criterion = criterion,min_samples_leaf=5,adaptive_complexity=True, alpha=0.5),
                "ABU3": AbuTree2(criterion = criterion,min_samples_leaf=5,adaptive_complexity=True, alpha=0.9),
...
# plt.tight_layout()
# plt.savefig(f"StableTrees_examples\plots\\example_mse_simulated_overtime_UAI__boston_with_BABU.png")
# plt.close()
...
for name, model in models.items():
    if name == "BABU":
        continue
    ax.plot(mse[name], stab[name] , label =r"$"+labels[name]+"$",linestyle='--', marker='o', markersize = 3,linewidth=1, c = colors[name])#np.arange(1,time+1,dtype=int)
    scatters+= [ax.scatter(x = x, y=y, s = 0.1, alpha=0) for (x,y) in zip(mse[name],stab[name])]
...
adjust_text(texts,add_objects=scatters,ax= ax)
plt.tight_layout()
plt.savefig(f"StableTrees_examples\plots\\example_mse_simulated_overtime_UAI_boston1.png")
plt.close()
